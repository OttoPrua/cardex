...
// 【BD-44 改名过渡期的 env 双读】claudego → bide 改名后，环境变量族一律以 BIDE_* 为主名、
...
// 为什么是 stderr 而不是 stdout：`bide list -json` 的 stdout 要能直接喂给解析器，
// 往里掺一行中文提示会把下游 json.Unmarshal 打红。
const (
	envRoot       = "BIDE_ROOT"
	envRootLegacy = "CLAUDEGO_ROOT"

	envRequireSyncScripts       = "BIDE_REQUIRE_SYNC_SCRIPTS"
	envRequireSyncScriptsLegacy = "CLAUDEGO_REQUIRE_SYNC_SCRIPTS"

	envSyncMirrorMode       = "BIDE_SYNC_MIRROR_MODE"
	envSyncMirrorModeLegacy = "CLAUDEGO_SYNC_MIRROR_MODE"

	envSyncLocalMirrorRoot       = "BIDE_SYNC_LOCAL_MIRROR_ROOT"
