...
install: build
	...
	rm -f $(PREFIX)/bide
	cp $(BIN) $(PREFIX)/bide
	@echo "已安装到 $(PREFIX)/bide（launchd 请在安装后重新运行 bide install-launchd）"

# install-shim: 旧名 claudego → 新名 bide 的兼容软链（BD-44 改名过渡期）。
...
# 用 ln -sf 而非 cp:软链跟随 bide 本体升级,不会留下一个永远停在旧版本的二进制副本。
