...
func applyTaskWriteDomain(t *Task, id, lineage, component, pathsCSV, resourcesCSV string) error {
	...
	norm, err := NormalizeWriteDomain(root, domain)
	if err != nil {
		return err
	}
	t.WriteDomain = &norm
...
// taskDurablyDone is the live DAG satisfaction predicate for this source
// base: a verifiable task status of done. This tree has no committed
// transition IDs; status done is the durable completion contract.
func taskDurablyDone(_ string, t *Task) bool {
	return t != nil && t.Status == statusDone
}
...
func liveDependencyNode(root string, t *Task) (DependencyNode, []WriteDomain, error) {
	...
	repo := taskRepoRoot(t)
	if repo == "" {
		return n, nil, errWriteDomainCanonicalization
	}
	norm, err := NormalizeWriteDomain(repo, *t.WriteDomain)
	if err != nil {
		return n, nil, err
	}
