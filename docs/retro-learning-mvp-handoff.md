...
2. Do not edit `retro_counter.json` or synthesize/replay 707. Wait until ten real business-task completions have accumulated after watermark 707 (two are already observed at `done_total=709`), then confirm `triggered_at >= 717` with a new `last_retro_task`.
