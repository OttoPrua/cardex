...
| `stakes_policy` | low=no review / normal=follow / high=force review + raise to high + cap 4 | per-card stakes → review-depth lookup (`add -stakes`); per-tier fields are `review` / `default_effort` / `max_fix_rounds` (`0` = follow the global cap). **Frozen onto the card at enqueue**, never re-read at run time — see [guide · stakes tiering](guide.en.md#per-card-stakes-tiering--stakes--review-depth-lookup-table) |
...
| `model_tiers` | {} (empty) | custom tier table: model ID (lowercase, exact or prefix match) → tier keyword (fable/opus/sonnet/haiku), overriding the built-in standard line. Lets fleets without stronger models rank by the cards they hold; affects tier display and engine-tier derivation only — dispatch routing never consumes tiers. Bad values rejected at load. See [guide · custom tiering](guide.en.md#custom-tiering-model_tiers-fleets-without-stronger-models-rank-by-the-cards-they-hold) |
