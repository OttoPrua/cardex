...
**进度双口径（页头「实发进度 / 预估进度」分段控件，与状态筛选芯片同排；委托人 2026-08-02
追加，同日两轮修正）**：默认的**实发进度**按已经派出去的卡计分母（12 卡完成 9 → 75%），但
cardex 的工作模式会持续派生新卡（review_after 复审、修复轮、emit 产出），实发口径常高估
完成度。切到**预估进度**后，分母换成预估最终总卡数（进度条尾部补一段斜纹「预估余量」幽灵段，
百分比带 `~` 后缀），预估来源两级：
- **计划锚点优先**：`board.json` 的 `projects.<id>.planned_total_cards`（阶段性计划总卡量）。
  计划达成/调整时人工更新它——这就是预估口径的校准 hook；被现存量超出时按现存计并提示更新。
- **派生耦合模型替补**：无锚点时按可测历史量估——**每完成 1 张卡平均派生 k 张新卡**
  （k = 系统派生卡/完成卡；系统派生 = 复审卡/修复轮卡/交叉派生腿/带 `emitted_by` 谱系标的
  emit 产出与收口/升级卡）。清完在途 A 张的全部后续 = A·k/(1−k)（等比级数，**把"派生的
  派生"一次性前置进预估**）。由此得到修正轮点名的趋势性质：完成不派生的卡 → 预估总量
  缩减；完成会派生的卡 → 派生量早已入账、总量基本不动——每次更新趋向完成，而不是分母
  越走越远。k≥0.85 属扩张期（装配浪进行中），按 0.85 给收敛**下限**并在 basis 明说。
  每次快照按最新历史**即时重算（自校准）**，零额度无定时任务；样本不足（完成 <5 或派生
  <3）显式回落现存卡数——预估必须带 basis，绝不给来历不明的百分比。诚实边界：从没发生
  过派生浪的年轻项目，第一波 emit 仍会抬升预估（没有历史就没有预知），那种场景请用计划
  锚点。存量系统派生卡缺 `emitted_by` 标会低估 k（方向=预估偏保守，basis 里披露）。

**第三口径「工时进度」（按工作量加权 + 预估完成时刻）**：卡数口径把每张卡当 1 份，但实测
本账本 1622 张 done 卡的中位 turns 是 **sequence 57 / design-review 34 / coordinate 8 /
progress-pull 2——相差 28 倍**，多步卡（112）又是单步卡（41）的 2.7 倍。于是"10 张完成 7 张
= 70%"在剩下三张全是 sequence 大卡时严重高估。工时口径改按每张卡的**工作量**算占比，并给出
预估完成时刻：

- **权重取 turns（回合数）作工作量代理**，不取时长——卡上没有执行时长字段，
  `updated_at−created_at` 大半是排队等待（见本页 ETA 一节的同一论证）。done 卡用实测 turns，
  未跑的卡按「同类型 × 是否多步」的历史**中位**预测（中位而非均值：turns 重尾右偏，
  均值会被少数超长卡拽高、让预测系统性偏大）；
- **完成时刻**用同一批样本实测的「每单位工作量耗多少墙钟分钟」换算，这个换算率天然吸收了
  并行度、限额冷却与红线时段。样本内跨度为零（算不出换算率）时**只给占比不给时间**；
- **已知缺口**：codex / 远端 / 引擎卡不回报 turns（实测 24% 的 done 卡 turns=0），这些卡按
  同类中位补估计值，偏差方向未知——覆盖率与该缺口都写在进度条下方与 basis 里；
- 项目实测样本 <12 张时整个口径判**不可用**，回落实发口径并把原因摆在明面上；
- 进度条与另外两个口径**同一套状态分段与状态色**（每段是"多少工作量处于该状态"，条尾同样有
  斜纹的预估余量段）。未跑的卡那几段是按同类中位预测的估计值——这一点由条下的实测覆盖率与
  basis 披露，不靠把段涂淡来暗示：分段讲的是"这部分活处于什么状态"，不是"这段数据多可靠"，
  两件事挤进同一个视觉通道会让两者都读不准。

**分桶随口径同步切换**：预估口径下，余量按**历史派生构成**分摊进设计/落地/修复/审核各桶
（历史派生卡里 60% 是修复就把 60% 的余量分给修复桶；最大余数法凑整，**Σ 桶余量 ≡ 项目余量**，
分桶与总条永远对得上账）；工时口径下，各桶给自己的加权分子分母（只计已存在的卡，预估余量
由总条承担——两处都摊会让两套口径的桶分母讲不同的故事）。没分到数的桶一律回落实发分母。
阶段条始终保持实发口径（阶段是执行切片，没有派生构成也没有独立的工作量基准可依）。
开关放在状态筛选芯片同一排（总览与项目页都有）——两者都是"看板怎么读"的视图开关，散在两处
会让人以为口径是某个页面的局部设置；切换偏好存 localStorage。
...
**为什么实测线尾部与旧预测线斜率不同**：旧字段 `burn_rate_pct_per_hour` 并不是“最后一段的当前速度”，而是当前额度周期内**全部样本**的普通最小二乘均速。前段空闲、后段突然并行时，实测折线尾部会很陡，整周期均速却仍被前段稀释；预测线再以这个均速从最新点出发，两条线自然不会相切。这不是 Cardex 与客户端各画了一条账。

现在每个账号窗口给两种明确标名的预测：

- **本周期均速外推**（`burn_rate_pct_per_hour` / `exhaust_at`）：继续使用当前周期全部样本，作为较稳定的基线并保持 API 兼容；
- **近期综合消耗外推**（`recent_burn_rate_pct_per_hour` / `recent_exhaust_at`）：从同一序列尾部取自适应 1–6 小时做拟合，`recent_span_minutes` 报真实参与跨度，对近期加速更敏感。两条有效预测中任一会在重置前归零，看板即预警，并把更早的时刻放进顶部摘要。

若预测会先于一个很远的周重置触底，图的横轴聚焦到两种归零预测，不再为画出数天后的重置线而把实测/预测挤在最左侧；图例仍明确报出“重置在图外”的具体时刻。没有重置前耗尽预测时，重置线照常进入横轴。

每条可信外推线与 0% 时间轴的交点都直接标注预计归零时间；两种时间靠得很近时自动错行，1 分钟内重合则合并成一个“均速/综合”标记。卡片的结论说明按“本周期均速 / 近期综合 / 窗口重置”分行，不再把速率、归零与重置时间挤成一整段。

这里的“综合”指**同一账号的全局额度读数**：CodexBar/usage feed 观测到的百分比天然包含 Cardex、Codex/Claude 客户端和其他使用该账号的会话；源数据没有调用归属，不能可靠拆成“Cardex 百分比线”和“客户端百分比线”。`task_spend` / `queue_spend` 仍保留 Cardex 自己的下限账，但量纲分别是 API 等价美元/加权 token，不能从全局百分比里相减来伪造一条外部消耗线。两种预测都是趋势参考，不是服务商的可用性保证；若额度先重置，则本周期不会按那条线变为不可用。
