...
# 用于验证 cardex 的 codexErrorLine 会跨过横幅取真错误。行为由 $MOCK_DIR/codex-plan 控制（每行一次调用）。
set -u
out=""; prev=""
...
# 横幅永远先打（真 codex 第一行就是它）——cardex 提取错误时必须跨过它。
printf 'Reading additional input from stdin...\nOpenAI Codex v0.144.1\n--------\nmodel: mock\nsandbox: read-only\n--------\n'

case "$beh" in
  netfail)
    # 瞬时网络错误：真错误在末尾，-o 不写（空结果）→ cardex 应取到真错误、判 transient 退避重试
