...
| Deployment binds its preimage | A later build from another worktree can silently replace an accepted binary before its natural canary. | Bind installation to a clean full commit and the expected current production SHA-256. Reject dirty builds and preimage drift before deleting the old inode. |
