...
**分桶随口径同步切换**：预估余量按**历史派生构成**分摊进设计/落地/修复/审核各桶（历史派生
卡里 60% 是修复就把 60% 的余量分给修复桶；最大余数法凑整，**Σ 桶余量 ≡ 项目余量**，分桶
与总条永远对得上账）；没分到余量的桶回落实发分母。阶段条保持实发口径（阶段是执行切片，
没有派生构成可依）。开关放在状态筛选芯片同一排（总览与项目页都有）——两者都是"看板怎么
读"的视图开关，散在两处会让人以为口径是某个页面的局部设置；切换偏好存 localStorage。
