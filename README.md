...
```bash
claudego add -title "重构鉴权" -dir ~/Projects/myapp -file steps.md   # 把活丢进队列
claudego install-launchd                                            # 之后它自己跑
```
...
| “现在到底跑到哪了” | `claudego list` + Web 看板（kanban / 额度燃尽 / 落地进度）+ 每张卡的事件账本 |
...
claudego doctor                # 自检：claude CLI、目录、配置
```
...
# ③ 手上的会话刚被限额打断：直接接管续跑（会话 id 用 claudego sessions 查）
...
```bash
claudego run                   # 先手动跑一轮验证
claudego install-launchd       # 后台调度：每 5 分钟 tick 一次，开机自启（macOS）
claudego list                  # 看板：标题列＝「标题 ▸ 最新进度」
claudego log <id>              # 某张卡的细节；cmd <id> 打印手动接管命令
claudego board                 # Web 看板 http://127.0.0.1:8787
```

非 macOS：`claudego daemon` 前台常驻，或让 systemd timer / cron / Windows 任务计划程序每 5 分钟拉一次 `claudego run`。核心是纯 Go，三大平台都能编译；单实例锁已跨平台，定时并发不会撞车。
...
1. **编排零额度**——调度器是纯 Go 本地代码，只读写 `~/.claudego` 下的 JSON 文件，自己不调用任何模型；派发、退避、看板、账本一律不花额度，只有任务真正执行时才 `claude -p`。
...
- **[Web 看板](docs/guide.md#web-看板board-命令)**——只读 kanban + 额度燃尽曲线 + 目标锚定的「落地进度」，数据不足一律显式披露，绝不编造估算。
...
| [配置参考](docs/config.md) | `~/.claudego/config.json` 全量键表 + 模板说明 |
