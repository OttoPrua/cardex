...
| Deployment includes every consumer | Binary hash and Web health do not prove scheduled execution. | Verify installed hash/signature, Board PID and `*:8788`, health, loaded `com.cardex.tick`, one fresh tick exit, then the first natural retrospective report. |
...
1. Close the current known defect: `cardex doctor` must reject a present-but-unexecutable launchd
   timer, including `OS_REASON_CODESIGNING` and `needs LWCR update`.
2. Keep the production retrospective gate at the natural `707` watermark. Do not edit the counter
   or synthesize completions. The first valid v2 report is the acceptance artifact.
3. After that report, choose at most one next packet. Current evidence makes usage coverage and
