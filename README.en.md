...
| `progress-pull` | Progress-pull session: `--resume` a session and have it emit a structured progress report to disk | Read-only; Haiku→Grok 4.6/medium (high only when explicitly quality-sensitive), with eligible continuation limited to Kimi or the already-proven OpenCode Go lightweight lane |
