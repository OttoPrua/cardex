...
- Cross cards are **read-only analysis** (read contracts/source/diffs, never write the repo); the codex side by default runs in a one-shot isolated copy + `--sandbox workspace-write`, and the copy is built and torn down per card so the source repo is never write-polluted (see the "Sandbox" section on CG-R3 `codex_review_sandbox`); `-dir` may not be the claudego data root or a subdirectory of it;
...
| `codex_review_sandbox` | "worktree-write" | Sandbox policy for codex read-only analysis cards (design-review/crosscheck etc.). Default `worktree-write` builds a one-shot isolated copy + `--sandbox workspace-write` so reviews can run tests / drop fixtures for dynamic verification; the copy lands in `<root>/tmp/codex-review-work/` and is torn down when the card ends — the source repo is never write-polluted (CG-R3). Set to `readonly` to roll back to the old behavior (codex runs `--sandbox read-only` directly in the source repo — static reading only). `sequence` cards are unaffected. |
