...
## 2026-08-10 · 燃尽预测拆清“本周期均速 / 近期综合消耗”

- 修正文案：既有 `burn_rate_pct_per_hour` 是当前额度周期全部样本的 OLS 均速，不再误称
  “当前速度”；这解释了近期突然加速时，实测折线尾部与外推线不相切。
- 新增近期综合线：同一账号全局额度序列尾部按窗口自适应回看 1–6 小时，返回
  `recent_burn_rate_pct_per_hour`、实际跨度与预计归零时刻；图上与本周期均速并列，顶部摘要
  报两种有效预测中更早的重置前触底时刻；周重置远晚于预测时，横轴聚焦归零区间并把
  重置具体时刻标成“图外”，不再把曲线压缩到左侧。
- 口径硬披露：账号百分比已混合 Cardex 与 Codex/Claude 客户端，源数据无法归因拆分；
  Cardex 自己的美元/token 下限账量纲不同，不拿它相减伪造“客户端消耗百分比”。
- 外推线抵达 0% 时间轴的交点直接标注预计归零时间；相近预测错行、近乎重合则合并。
  “将在重置前烧完”等结论的详情按均速、近期综合、窗口重置分行展示。
